...
function getNewsAndWeather() {
  ...
  return fetch(newsURL)//newsurl로부터 정보를 요청한다. 프로미스 객체의 형태
    ...
    //promiseresult가 response -> object로 변함
...
/*다른 풀이
function getNewsAndWeather() { //구조분해할당
  return fetch(newsURL)
    .then ((res) => res.json())
      .then(({data:news}) => {
        return fetch(weatherURL)
          .then((res) => res.json())
          .then((weather) => {return {news, weather};})
        })
}
*/